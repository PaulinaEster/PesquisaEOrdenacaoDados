# 2. Altere o programa apresentado anteriormente para:
# ✓ Possibilitar a inserção, remoção e pesquisa de objetos
# • Na implementação, utilize as classes Pessoa e Gerador,
# apresentadas nas aulas anteriores 




# Possível implementação de uma árvore binária de busca. Disponível em:
# https://gist.github.com/divanibarbosa/a8662693e44ab9ee0d0e8c2d74808929
# Impedir que chaves iguais sejam inseridas
# ✓ Permitir a geração de árvores com valores aleatórios
# • Passar como parâmetro
#   – altura desejada (é o comprimento do caminho da raiz até ao nó de profundidade
#   mais elevada.)
#   – se a árvore será perfeita ou não (é uma árvore binária completa cujas folhas se
#   encontram todas ao mesmo nível). Uma árvore binária completa é uma árvore
#   binária cujos níveis, exceptuando eventualmente o último, estão completamente
#   preenchidos e, no último nível, os nós encontram-se o mais à esquerda possível.
# ✓ Exibir a árvore em modo gráfico (texto e imagem)
# ✓ Converter a árvore para uma lista
...
class Tree:
    ...
    # Impedir que chaves iguais sejam inseridas
    def inserir(self, v):
        if arv.buscar(v) != None:
            print("Erro: valor já foi inserido na arvore.")
        else:
    ...
    def remover(self, v):
        ...
        while atual.item != v:  # enquanto nao encontrou
            pai = atual
            if v < atual.item.getId():  # caminha para esquerda
                atual = atual.esq
    ...
    def inOrder(self, atual):
        if atual != None:
            ...
            print(atual.item.getRegistro(), end=" ")
    ...
    def caminhar(self):
        ...
        if self.root != None:  # se arvore nao esta vazia
            print(" Valor minimo: %d" % (self.minn().item.getId()))
            print(" Valor maximo: %d" % (self.maxx().item.getId()))

    
# ✓ Converter a árvore para uma lista

    def converteLista(self):
...
#   mais elevada.)
    ...
    def geraLista(self):
        tamanho = int(input("Insira a altura desejada:  "))
        g = Gerador()
        while tamanho >= self.altura(self.root):
            valor = Pessoa(randint(1,1000), g.geraSobrenome(), g.geraNome(), g.geraIdade())
            arv.inserir(valor)
...
while opcao != 5:
    ...
    print(" --- 6: Converte para lista")
    print(" --- 7: Gera lista Aleatoria")
    print("***********************************")
    opcao = int(input("-> "))
    if opcao == 2:
